...
export function Logo({ size = 30, variant = 'default' }: Props) {
  return match(variant)
    ...
    .with('icon', () => <Icon size={size} />)
    .exhaustive()
}

function Icon({ size = 30 }: Props) {
  const idA = useId()
  const idB = useId()
  ...
  return (
    ...
    >
      <defs>
        <linearGradient x1="-.828%" y1="7.652%" x2="57.636%" y2="78.411%" id={idA}>
          <stop stopColor="#41D1FF" offset="0%" />
          <stop stopColor="#BD34FE" offset="100%" />
        </linearGradient>
        <linearGradient x1="43.376%" y1="2.242%" x2="50.316%" y2="89.03%" id={idB}>
